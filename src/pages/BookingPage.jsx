...
import ParticipantDetailsSection from '../components/booking/ParticipantDetailsSection';
...
function buildParticipantRows(cart, tickets, previousRows = []) {
  const next = [];
...
function validateParticipantRows(rows) {
...
export default function BookingPage() {
  ...
  const totalTickets = useMemo(() => Object.values(cart).reduce((sum, qty) => sum + qty, 0), [cart]);
  const rawTotal = useMemo(() => tickets.reduce((sum, ticket) => sum + (cart[ticket.id] || 0) * Number(ticket.price || 0), 0), [cart, tickets]);
