...
export default function Payments() {
  ...
  const fetchPayments = async () => {
    try {
      // Fetch larger set to fill the dashboard appropriately
      const res = await axiosInstance.get('/payments/my-payments?size=100');
        ...
        <table className="dashboard-table">
          ...
          <tbody>
            {filteredPayments.map(p => (
              <tr key={p.id}>
                <td style={{ fontWeight: 600 }}>REF-{p.id}</td>
